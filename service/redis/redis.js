...
const readNotificationListHandler = redis.createClient();
...
exports.publishNotification = async function(userEmail, notification) {
    ...
    try {
        ...
        await publisher.publish(channelId, JSON.stringify({type:'NEW',message: notification['amount']}));
    }catch(error) {
        console.error(error);
    }finally {
...
exports.getRedisNotification = async function(userEmail) {
    ...
    console.log('getRedisNotification');
    try {
        ...
        const notificationList = await readNotificationListHandler.lRange(channelId, 0, -1);
        if(notificationList.length === 0){
            ...
            for(item of notificationList) {
                await readNotificationListHandler.lPush(channelId,JSON.stringify(item))
                // notifyList.push(channelId,JSON.stringify(item));
                notifyList = await readNotificationListHandler.lRange(channelId,0, -1)
            }
