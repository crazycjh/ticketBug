...
exports.dayliyPushNotification = async function() {
    ...
    const notifyList = await ticketNotifyList.findAll({
        attributes:['id','createDate','airport_1', 'airport_2', 'airport_3', 'airport_4', 'date_1', 'date_2', 'type', 'price' ],
                ...
                if(notifyUsers.length > 0) {
                    ...
                    for(user of notifyUsers){
                        ...
                        if( pushWaitingList[user.email]) {
                            ...
                            pushWaitingList[user.email]['amount'] =  pushWaitingList[user.email]['amount'] + 1;
                            pushWaitingList[user.email]['notification'].push(item); 
                        }else {
                            pushWaitingList[user.email] = {amount:1, notification:[item]};
                        }
